...
class Filterbank(object):
    ...
    def gmrt_psr_tool(self, template_path):
        ...
        if self.config.backend == "GWB":
            cmd_gptool = (
                "/data/jroy/softwares/gptool_ver4.4.5.PSR/gptool -f {} -o {} -m {} -nodedisp -zsub "
                "-t 2"
    ...
    def mask_RFI(self):
        ...
        """ Construct an RFI mask for the filterbank file using the "rfifind" module. """

        self.logger.info("Start constructing the RFI mask.")

        rfi_mask_name = os.path.join(
            self.output_rfi_path, self.output_name + ".rfifind"
        )

        if self.config.backend == "GWB" or self.config.backend == "SIM":

            cmd_rfimask = (
                "rfifind -blocks 1 -zapchan {} -freqsig {} {} -o " "{}"
            ).format(
                self.config.bad_channels, self.config.freq_sig, self.path, rfi_mask_name
            )
        else:

            cmd_rfimask = "rfifind -blocks 1 -zapchan {} {} -o {}".format(
                self.config.bad_channels, self.path, rfi_mask_name
            )
        arg_rfimask = shlex.split(cmd_rfimask)
        proc_rfimask = subprocess.Popen(
            arg_rfimask, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
        )
        proc_rfimask.wait()

        self.logger.info("RFI mask constructed.")
    ...
    def segmented_dedisp(self, dm_segment):
        ...
        rfi_mask_path = os.path.join(
            self.output_rfi_path, self.output_name + ".rfifind_rfifind.mask"
        )
    ...
    def segmented_dedisp1(self, dm_segment):
        ...
        #        rfi_mask_path = os.path.join(self.output_rfi_path, self.output_name + '.rfifind_rfifind.mask')
    ...
    def para_dedisp(self):
        ...
        with Pool() as pool:
            [
                p for p in pool.map(self.segmented_dedisp1, self.dm_segments)
            ]  # Use segmented_dedisp1 if you are not using RFI masks, otherwise use segmented_dedisp
    ...
    def process(self):
        ...
        self.make_dirs()
        self.load_header()
        #        self.mask_RFI()                    # comment out this part if you are having problems in prepsubband related to RFI masks, along with this you need to change prepsubband syntax
        self.para_dedisp()
        self.fast_folding()
